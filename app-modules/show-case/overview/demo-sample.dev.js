(function () {
	angular.module("App").controller("demoCtrl", controller);
	...
	function controller($scope, apiAction, $adapter, dataStore, stateParams, $timeout, dataSet, $element) {
		let initialConfig = {
			...
			php: {
				php: true,
				action: function (limit) {
					return "getOrders.php?limit=" + limit;
				},
				adapter: {
					conversions: {
						date: {
					...
					parsingCollection: true,
					//creates invariant columns (without accents, non alphanumeric characters) for ordering and filtering
					invariant: ["customer", "deliveryCity", "deliveryCountry"]
				}
			}
		};
		let config = stateParams.type === "php" ? initialConfig.php : initialConfig.json;
