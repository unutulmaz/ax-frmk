...
angular.module('amChartsDirective', []).directive('amChart', ['$q', function ($q) {
	return {
		...
		link: function ($scope, $el) {
			...
			$el.attr('id', id);
			var chart;
			var checkOptions = function (loaded) {
				// we can't render a chart without any data
				if (!loaded) return;
				if ($scope.options.loaded) {
					var renderChart = function (amChartOptions) {
						...
						var theme = o.theme?AmCharts.themes[o.theme]:null;
						if (o.type === 'xy') chart = new AmCharts.AmXYChart(theme);
						else if (o.type === 'pie') chart = new AmCharts.AmPieChart(theme);
						else if (o.type === 'funnel') chart = new AmCharts.AmFunnelChart(theme);
						else if (o.type === 'radar') chart = new AmCharts.AmRadarChart(theme);
						else if (o.type === 'gauge') chart = AmCharts.AmAngularGauge(theme);
						else chart = new AmCharts.AmSerialChart(theme);
						
						$scope.options.chart = function () { return chart; };
						...
						$q.when(o.data)
							.then(function (data) {
									...
									if (o.categoryAxis) {
										...
										if (categoryAxis) {
											...
											for (var i = 0; i < keys.length; i++) {
												if (!angular.isObject(o.categoryAxis[keys[i]]) ||
									...
									for (var i = 0; i < obj.length; i++) {
										if (obj[i].listeners) {
											...
											for (var l = 0; l < listeners.length; l++) {
												chartObj[i].addListener(listeners[l].event,
													listeners[l].method);
											}
										}
									}
								...
								for (var i = 0; i < chartKeys.length; i++) {
									...
									if (typeof o[chartKey] !== 'object' &&
										typeof o[chartKey] !== 'function') {
										chart[chartKey] = o[chartKey];
									} else if (typeof o[chartKey] === 'object') {
										addEventListeners(o[chartKey], chart[chartKey]);
									}
								}
								//if (chart.legend) chart.addLegend(chart.legend);
								// WRITE
								chart.write(id);
								$scope.chart = chart;
							...
							if (id === $el[0].id || !id) {
								if ($scope.options.type == 'gauge') {
									...
									for (var i = 0; i < data.length; i++) {
										chart.arrows[i] && chart.arrows[i].setValue && chart.arrows[i].setValue(data[i]);//jshint ignore:line
									}
								} else {
									chart.dataProvider = data;
									chart.validateData();
					...
					$scope.$on('$destroy',
						function () {
							if (chart.chartDiv.isConnected) chart.clear();
			...
			$scope.$watch('options.loaded', checkOptions);
