...
DIFAT chains by storing the next sector number as the last 32 bits */
		...
		function rebuild_cfb(cfb, f) {
			...
			for (i = 0; i < data.length; ++i) {
				...
				if (i === 0) {
					...
					if (dirname(cfb.FullPaths[i + 1] || "") == dirname(nm))
						elt.R = i + 1;
					elt.type = 2;
				}
			}
		...
		function find(cfb, path) {
			...
			for (w = 0; w < UCFullPaths.length; ++w) {
				if (UCFullPaths[w].replace(chr0, '').replace(chr1, '!') == UCPath)
					return cfb.FileIndex[w];
				if (UCPaths[w].replace(chr0, '').replace(chr1, '!') == UCPath)
					return cfb.FileIndex[w];
			}
			return null;
		}
	...
	if (has_buf) {
		...
		__utf8 = function utf8_b(b, s, e) {
			return (Buffer.isBuffer(b)) ? b.toString('utf8', s, e) : __utf8(b, s, e);
		}
			;
	...
	/* [MS-XLS] 2.5.161 */
	/* [MS-XLSB] 2.5.75 */
	var XLSIcv = rgbify([/* Color Constants */
	...
	var type2ct /*{[string]:Array<string>}*/
		= evert_arr(ct2type);
	...
	var CTYPE_DEFAULTS = [['xml', 'application/xml'], ['bin', 'application/vnd.ms-excel.sheet.binary.macroEnabled.main'], ['vml', 'application/vnd.openxmlformats-officedocument.vmlDrawing'], /* from test files */
		...
		if (Custprops)
			keys(Custprops).forEach(function (k) {
				if (!Custprops.hasOwnProperty(k))
					return;
				if (Props && Props.hasOwnProperty(k))
					return;
				var m = Custprops[k];
				var t = "string";
				if (typeof m == 'number') {
	...
	function parse_TypedPropertyValue(blob, type, _opts) {
		...
		switch (type === VT_VARIANT ? t : type) {
			...
			case 0x51: /*VT_USTR*/
				...
				return parse_VtUnalignedString(blob, t /*, 4*/
				).replace(chr0, '');
	...
	function parse_XLUnicodeRichExtendedString(blob) {
		...
		var fExtSt = flags & 0x4, fRichSt = flags & 0x8;
	...
	function write_BOF(wb, t, o) {
		...
		out.write_shift(2, t);
		if (w > 4)
			out.write_shift(2, 0x7262);
		if (w > 6)
			out.write_shift(2, 0x07CD);
		if (w > 8) {
	...
	function parse_Window2(blob, length, opts) {
		if (opts && opts.biff >= 2 && opts.biff < 8)
			return {};
	...
	var DBF = (function () {
		...
		function sheet_to_dbf(ws, opts) {
			...
			for (i = 0; i < 4; ++i)
				h.write_shift(4, 0);
			h.write_shift(4, 0x00000300);
			// TODO: CP

			for (i = 0,
				j = 0; i < headers.length; ++i) {
				if (headers[i] == null)
					continue;
				var hf = ba.next(32);
	...
	var PRN = (function () {
		...
		function prn_to_sheet(d, opts) {
			...
			else if ((opts.type == 'binary' || opts.type == 'buffer') && typeof cptable !== 'undefined' && opts.codepage)
		...
		function sheet_to_rtf(ws) {
			...
			for (var R = r.s.r; R <= r.e.r; ++R) {
				...
				for (C = r.s.c; C <= r.e.c; ++C) {
					...
					if (!cell || cell.v == null && (!cell.f || cell.F))
						continue;
					o.push(" " + (cell.w || (format_cell(cell),
						cell.w)));
					o.push("\\cell");
				}
...
*/
	...
	function parse_ws_xml_sheetpr(sheetPr, s, wb, idx) {
		...
		if (!wb.Sheets[idx])
			wb.Sheets[idx] = {};
		if (data.codeName)
			wb.Sheets[idx].CodeName = data.codeName;
	}
	...
	function write_ws_xml_sheetviews(ws, opts, idx, wb) {
		...
		var kids = [], tag;
		if (ws['!freeze']) {
			tag = writextag('pane', null, ws['!freeze']);
			kids.push(tag);

			//tag = writextag('selection', null, {pane: "topLeft"});
			//kids.push(tag);

			//tag = writextag('selection', null, {pane: "bottomLeft"});
			//kids.push(tag);
		...
		//let pane = (ws["!freeze"] !== null) ? writextag("pane", null, ws["!freeze"]) : null;
		tag = writextag("sheetViews", writextag("sheetView", kids.join('') || undefined, sview), {});
		console.log("sheetViews", tag);
		return tag;
	}
	...
	function write_BrtSheetProtection(sp, o) {
		...
		o.write_shift(4, 1);
		// this record should not be written if no protection
	...
	function write_WSFMTINFO() {/* [ACWSFMTINFO] */
		...
		data.replace(tagregex, function xml_wb(x, idx) {
			...
			switch (strip_ns(y[0])) {
				...
				case '<workbookPr/>':
					...
					if (y.codeName)
						wb.WBProps.CodeName = y.codeName;
					break;
				case '</workbookPr>':
					break;
...
return o;
	...
	function parse_xlink(data, name, opts) {
		if (name.slice(-4) === ".bin")
			return parse_xlink_bin((data), name, opts);
				...
				default:
					...
					if (state[state.length - 1][0] === 'CustomDocumentProperties') {
						if (Rn[0].slice(-2) === "/>")
							...
							xlml_set_custprop(Custprops, Rn[3], cp, str.slice(pidx, Rn.index));
						else {
							cp = Rn;
							pidx = Rn.index + Rn[0].length;
						}
	...
	function write_ws_xlml_wsopts(ws, opts, idx, wb) {
		...
		if (ws['!margins']) {
			...
			if (ws['!margins'].header)
				o.push(writextag("Header", null, {
					'x:Margin': ws['!margins'].header
				}));
			if (ws['!margins'].footer)
				o.push(writextag("Footer", null, {
...
*/
	...
	function parse_xlscfb(cfb, options) {
		...
		if (options.bookProps && !options.bookSheets)
			...
			/* Quattro Pro 7-8 */
				...
				WorkbookP = WK_.to_workbook(_data.content, (options.type = T,
					options));
			/* Quattro Pro 9 */
	...
	function write_xlscfb(wb, opts) {
		...
		if (o.biff == 8 && wb.vbaraw)
			fill_vba_xls(cfb, CFB.read(wb.vbaraw, {
	...
	var HTML_ = (function () {
		function html_to_sheet(str, _opts) {
			...
			for (i = 0; i < rows.length; ++i) {
				...
				for (j = 0; j < cells.length; ++j) {
					...
					if (cell.slice(0, 3).toLowerCase() != "<td")
						continue;
					var m = cell, cc = 0;
		...
		function make_html_row(ws, r, R, o) {
			...
			for (var C = r.s.c; C <= r.e.c; ++C) {
				...
				/* TODO: html entities */
				var w = cell.h || escapexml(cell.w || (format_cell(cell),
					cell.w) || "");
				var sp = {};
				if (RS > 1)
					sp.rowspan = RS;
...
break; */
	...
	function write_cfb_type(wb, opts) {
		...
		switch (o.type) {
			...
			case "array":
				...
				break;
			case "file":
				return _fs.writeFileSync(o.file, CFB.write(cfb, {
					type: 'buffer'
				}));
			case "string":
				throw new Error("'string' output type invalid for '" + o.bookType + ' files');
	...
	function sheet_to_json(sheet, opts) {
		if (sheet == null || sheet["!ref"] == null)
			return [];
		var val = { t: 'n', v: 0 };
		var header = 0, offset = 1, hdr = [], isempty = true, v = 0, vv = "";
		var r = {
			s: { r: 0, c: 0 },
			e: {
				r: 0, c: 0
	...
	function sheet_add_json(_ws, js, opts) {
		...
		js.forEach(function (JS, R) {
			keys(JS).filter(function (x) {
				return JS.hasOwnProperty(x);
			}).forEach(function (k) {
